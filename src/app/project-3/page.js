...
export default function Project3Page() {
  ...
  useEffect(() => {
    console.log('[Project 3] Component mounted');
    ...
    const loadScripts = async () => {
      if (scriptsLoaded) {
        console.log('[Project 3] Scripts already loaded, skipping');
        return () => {};
      }
      
      try {
        console.log('[Project 3] Loading scripts...');
        scriptsLoaded = true;
        
        if (!window.lenis) {
          const lenisModule = await import('@/lib/scripts/lenis-scroll.js');
          lenisModule.initLenis();
          console.log('[Project 3] Lenis loaded');
        }
        
        const projectModule = await import('@/lib/scripts/project.js');
        projectModule.initProject();
        console.log('[Project 3] Project initialized');
        
        return () => {
          console.log('[Project 3] Cleanup');
          scriptsLoaded = false;
        };
      } catch (error) {
        console.error('[Project 3] Error loading scripts:', error);
        scriptsLoaded = false;
        return () => {};
      }
    };

    console.log('[Project 3] Waiting for transition to complete');
    
    const handleTransitionComplete = async () => {
      console.log('[Project 3] Transition complete, loading scripts');
    ...
    const fallbackTimer = setTimeout(async () => {
      console.log('[Project 3] Fallback timeout');
    ...
    return () => {
      console.log('[Project 3] Component unmounting');
