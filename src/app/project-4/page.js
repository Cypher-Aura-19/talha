...
export default function Project4Page() {
  const project = getProjectById("4");

  useEffect(() => {
    console.log('[Project 4] Component mounted');
    ...
    const loadScripts = async () => {
      if (scriptsLoaded) {
        console.log('[Project 4] Scripts already loaded, skipping');
        return () => {};
      }
      
      try {
        console.log('[Project 4] Loading scripts...');
        scriptsLoaded = true;
        
        if (!window.lenis) {
          const lenisModule = await import('@/lib/scripts/lenis-scroll.js');
          lenisModule.initLenis();
          console.log('[Project 4] Lenis loaded');
        }
        
        const projectModule = await import('@/lib/scripts/project.js');
        projectModule.initProject();
        console.log('[Project 4] Project initialized');
        
        return () => {
          console.log('[Project 4] Cleanup');
          scriptsLoaded = false;
        };
      } catch (error) {
        console.error('[Project 4] Error loading scripts:', error);
        scriptsLoaded = false;
        return () => {};
      }
    };

    console.log('[Project 4] Waiting for transition to complete');
    
    const handleTransitionComplete = async () => {
      console.log('[Project 4] Transition complete, loading scripts');
    ...
    const fallbackTimer = setTimeout(async () => {
      console.log('[Project 4] Fallback timeout');
    ...
    return () => {
      console.log('[Project 4] Component unmounting');
